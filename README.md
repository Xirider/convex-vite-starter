...
```bash
# Required - Convex (auto-generated by `bunx convex dev`)
...
VIKTOR_SPACES_API_URL=https://your-api-url
...
Environment variables must be set on the Convex deployment for your backend functions to access them at runtime.
...
1. **Set on Convex deployment** (for production):
   ```bash
